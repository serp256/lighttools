package {
	...
	public class Quest extends Sprite {
		...
		public function Quest() {
			...
			mainpanel.add(lb, {left:50, top:25});
			...
			mainpanel.add(button, {left:20, top:20});
		...
		private function completeHandler(e:Event):void{
			...
			var vBox:VBox = new VBox(null, true);
			vBox.addList(questBns);
			mainpanel.add(vBox, {left:20, top:20});
			...
			mainpanel.add(button, {left:280, top:20});
			...
			mainpanel.add(button, {left:400, top:20});
		...
		private function onClickQuest(e:*):void {
			...
			questPanel = new VBox(null, false, 15, VBox.TL_ALIGN);
			...
			if (prev.length){
				...
				lb.setLayout({w:200});
				prev.unshift(lb);
				var vb:VBox = new VBox(null, true);
				vb.addList(prev);
				questPanel.add(vb);
			}

			button = createQuestButton(cur.data, 'VToolBlueButtonBg');
			vb = new VBox(null, true);
		...
		private function createQuestButton(data:*, skin:String = 'VToolGreenButtonBg'):VButton{
			var button:VButton = UIFactory.createButton(AssetManager.getEmbedSkin(skin, VSkin.STRETCH), {w:200, h:30, vCenter:0, hCenter:0}, new VLabel(
